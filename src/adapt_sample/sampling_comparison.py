...
class SamplingComparison:
    def __init__(
        ...
        # BEACON BOS prior parameters (Normal-Inverse-Gamma)
        alpha0: float = -0.5,
        nu0: float = 0.0,
        beta0: float = 0.0,
        mu0: float = 0.0,
        # BEACON sampling parameters
        k0_bos: int = 3,
        adaptive_ignore: bool = True,
        ...
        Args:
            n_total:         Maximum samples per prompt (horizon for BOS DP).
            results_file:    Path to evaluation results JSON (optional in debug mode).
            alpha0, nu0, beta0, mu0:
                             Normal-Inverse-Gamma prior parameters for BEACON.
                             alpha0=-0.5, nu0=0 gives a non-informative (improper) prior.
            k0_bos:          Initial samples consumed before the BOS stopping rule
                             starts evaluating (must be >= 3 for posterior to be defined).
            adaptive_ignore: If True, down-weight extreme low-reward observations
                             when updating the posterior (more robust to outliers).
            adaptive_alpha:  Tail probability threshold for the adaptive-ignore rule.
            grid_size:       Resolution of the pre-computed h-index DP look-up table.
        """
        # BEACON BOS hyperparameters
        self.alpha0 = alpha0
        self.nu0 = nu0
        self.beta0 = beta0
        self.mu0 = mu0
        self.k0_bos = max(3, k0_bos)   # posterior requires at least 3 observations
        self.adaptive_ignore = adaptive_ignore
    ...
    def _run_bos_sampling(self, responses: List[Dict], cost_threshold: float, use_myopic_h: bool) -> Dict:
        ...
        # Fallback: if fewer responses than k0_bos, use Best-of-N among available
        if n_total < self.k0_bos:
        ...
        for i in range(self.k0_bos, n_total):
            ...
            if h_val <= c_value:
                ...
                z_k, mu_k, sigma_k = update_parameters(
                    ...
                    use_adaptive_ignore=self.adaptive_ignore,
    ...
    # --- AdaBoN Baseline: Adaptive Best-of-N (arXiv 2505.12050) ---
    @staticmethod
    def _estimate_difficulty(rewards: List[float], method: str = "variance") -> float:
        """
        Scalar difficulty score from a prompt's initial reward samples.
          variance   : Var(rewards)      — high spread  = uncertain
          max_reward : 1 - max(rewards)  — low best     = harder
          combined   : product of both
        """
        if len(rewards) < 1:
            return 0.0
        if method == "variance":
            return float(np.var(rewards, ddof=min(1, len(rewards) - 1)))
        elif method == "max_reward":
            return max(0.0, 1.0 - float(np.max(rewards)))
        elif method == "combined":
            v = float(np.var(rewards, ddof=min(1, len(rewards) - 1)))
            p = max(0.0, 1.0 - float(np.max(rewards)))
            return v * p
        raise ValueError(f"Unknown difficulty method: {method}")

    def _compute_adabon_allocations(
        self,
        all_initial_rewards: List[List[float]],
        k0: int,
        total_budget: int,
        difficulty_method: str = "variance",
    ) -> List[int]:
        """
        AdaBoN Phase-2: distribute remaining budget proportionally to difficulty.
        Returns per-prompt total sample count (>= k0).
        """
        num_prompts = len(all_initial_rewards)
        remaining = max(0, (total_budget - k0) * num_prompts)
        difficulties = [self._estimate_difficulty(r, difficulty_method) for r in all_initial_rewards]
        total_d = sum(difficulties)
        allocations = []
        for d in difficulties:
            extra = (remaining * d / total_d) if total_d > 0 else (remaining / num_prompts)
            allocations.append(k0 + int(math.ceil(extra)))
        return allocations

    # --- Comparison Framework ---
    def compare_methods(
        self,
        cost_thresholds: List[float] = [0.001, 0.01, 0.05, 0.1, 0.5, 1.0, 2.0, 5.0],
        adabon_k0: int = 3,
        adabon_budgets: List[int] = None,
        adabon_difficulty: str = "variance",
    ) -> Dict:
        """
        Compare BEACON (main method) against all baselines.

        Baselines
        ---------
        random          : pick 1 sample uniformly at random
        self_consistency: majority vote over all samples
        best_of_n       : oracle pick from all samples (upper bound)
        adabon          : Adaptive Best-of-N (arXiv 2505.12050) —
                          explores k0 samples per prompt, then
                          reallocates remaining budget proportionally
                          to per-prompt difficulty

        Main method
        -----------
        dynamic (BEACON): Bayesian Optimal Stopping with full DP h-index
        """
        if adabon_budgets is None:
            adabon_budgets = [4, 8, 16, self.n_total]
        valid_adabon_budgets = [b for b in adabon_budgets if b >= adabon_k0]
        ...
        results = {
            ...
            "adabon":  {b: {"accuracy": [], "avg_reward": [], "samples_used": [], "value": []} for b in valid_adabon_budgets},
        }

        # Pre-compute AdaBoN per-prompt allocations (requires a global pass first)
        all_questions = self.results.get("detailed_results", [])
        all_initial_rewards = []
        for q in all_questions:
            resp = q.get("responses", [])
            rews = [r["reward_scores"]["nemotron"] for r in resp[:adabon_k0]]
            all_initial_rewards.append(rews)

        adabon_alloc = {}
        for budget in valid_adabon_budgets:
            adabon_alloc[budget] = self._compute_adabon_allocations(
                all_initial_rewards, adabon_k0, budget, adabon_difficulty
            )
        ...
        for q_idx, question_data in enumerate(tqdm(all_questions, desc="Processing questions")):
            ...
            # BEACON: dynamic Bayesian Optimal Stopping
            ...
            # AdaBoN baseline
            for budget in valid_adabon_budgets:
                n_i = min(adabon_alloc[budget][q_idx], len(responses))
                if n_i == 0:
                    continue
                subset_rewards = [r["reward_scores"]["nemotron"] for r in responses[:n_i]]
                best_idx = int(np.argmax(subset_rewards))
                best_resp = responses[best_idx]
                results["adabon"][budget]["accuracy"].append(1.0 if best_resp["is_correct"] else 0.0)
                results["adabon"][budget]["avg_reward"].append(best_resp["reward_scores"]["nemotron"])
                results["adabon"][budget]["samples_used"].append(n_i)
                results["adabon"][budget]["value"].append(
                    best_resp["reward_scores"]["nemotron"] - n_i * avg_cost
                )

        # Aggregate
        for method, method_results in results.items():
            if method in ("dynamic", "adabon"):
                for key, key_results in method_results.items():
                    for stat in ("accuracy", "avg_reward", "samples_used", "value"):
                        vals = key_results[stat]
                        results[method][key][stat] = np.mean(vals) if vals else 0.0
            else:
                for stat in ("accuracy", "avg_reward", "samples_used", "value"):
                    vals = method_results[stat]
                    results[method][stat] = np.mean(vals) if vals else 0.0
    ...
    def plot_results(self, results: Dict, output_dir: str):
        """
        Plot BEACON vs all baselines: accuracy, reward, and value vs. average samples used.
        """
        ...
        adabon_samples   = [results["adabon"][b]["samples_used"]   for b in sorted(results["adabon"].keys())]
        adabon_accuracy  = [results["adabon"][b]["accuracy"]       for b in sorted(results["adabon"].keys())]
        adabon_rewards   = [results["adabon"][b]["avg_reward"]     for b in sorted(results["adabon"].keys())]
        adabon_values    = [results["adabon"][b]["value"]          for b in sorted(results["adabon"].keys())]

        # ── Accuracy vs samples ──────────────────────────────────────────────
        plt.figure(figsize=(12, 7))
        plt.scatter(results["random"]["samples_used"], results["random"]["accuracy"],
                    label='Random (1 sample)', color='blue', s=100, marker='s', zorder=5)
        plt.scatter(results["best_of_n"]["samples_used"], results["best_of_n"]["accuracy"],
                    label=f'Best-of-N ({results["best_of_n"]["samples_used"]:.1f} samples)',
                    color='green', s=100, marker='^', zorder=5)
        plt.scatter(results["self_consistency"]["samples_used"], results["self_consistency"]["accuracy"],
                    label=f'Self-Consistency ({results["self_consistency"]["samples_used"]:.1f} samples)',
                    color='red', s=100, marker='o', zorder=5)
        if adabon_samples:
            plt.plot(adabon_samples, adabon_accuracy,
                     's--', label='AdaBoN (baseline)', color='orange', linewidth=2)
        if dynamic_samples:
            plt.plot(dynamic_samples, dynamic_accuracy,
                     'o-', label='BEACON (ours)', color='purple', linewidth=2)
        plt.xlabel('Average Samples Used', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.title('BEACON vs Baselines: Accuracy vs. Samples Used', fontsize=14)
        ...
        plt.scatter(results["self_consistency"]["samples_used"], results["self_consistency"]["avg_reward"],
                    ...
                    color='red', s=100, marker='o', zorder=5)
        if adabon_samples:
            plt.plot(adabon_samples, adabon_rewards,
                     's--', label='AdaBoN (baseline)', color='orange', linewidth=2)
        if dynamic_samples:
            plt.plot(dynamic_samples, dynamic_rewards,
                     'o-', label='BEACON (ours)', color='purple', linewidth=2)
        plt.xlabel('Average Samples Used', fontsize=12)
        plt.ylabel('Average Nemotron Reward', fontsize=12)
        plt.title('BEACON vs Baselines: Reward vs. Samples Used', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.tight_layout()
        ...
        plt.scatter(results["self_consistency"]["samples_used"], results["self_consistency"]["value"],
                    ...
                    color='red', s=100, marker='o', zorder=5)
        if adabon_samples:
            plt.plot(adabon_samples, adabon_values,
                     's--', label='AdaBoN (baseline)', color='orange', linewidth=2)
        if dynamic_samples:
            plt.plot(dynamic_samples, dynamic_values,
                     'o-', label='BEACON (ours)', color='purple', linewidth=2)
        plt.xlabel('Average Samples Used', fontsize=12)
        plt.ylabel('Value Function (Reward − Samples × Cost)', fontsize=12)
        plt.title('BEACON vs Baselines: Value Function vs. Samples Used', fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.tight_layout()
...
def main():
    ...
    parser.add_argument("--n_total", type=int, required=True,
                      help="Maximum samples per prompt (BOS horizon)")
    # --- BEACON hyperparameters ---
    parser.add_argument("--beacon_alpha0", type=float, default=-0.5,
                      help="NIG prior: alpha0 (default: -0.5, non-informative)")
    parser.add_argument("--beacon_nu0", type=float, default=0.0,
                      help="NIG prior: nu0 (default: 0.0, non-informative)")
    parser.add_argument("--beacon_beta0", type=float, default=0.0,
                      help="NIG prior: beta0 (default: 0.0)")
    parser.add_argument("--beacon_mu0", type=float, default=0.0,
                      help="NIG prior: mu0, prior mean of reward (default: 0.0)")
    parser.add_argument("--beacon_k0", type=int, default=3,
                      help="Initial samples before BOS rule activates (default: 3, min: 3)")
    parser.add_argument("--beacon_adaptive_ignore", action="store_true", default=True,
                      help="Use adaptive-ignore update to down-weight outliers (default: True)")
    parser.add_argument("--beacon_no_adaptive_ignore", dest="beacon_adaptive_ignore",
                      action="store_false",
                      help="Disable adaptive-ignore; use standard posterior updates")
    parser.add_argument("--beacon_adaptive_alpha", type=float, default=0.01,
                      help="Tail probability threshold for adaptive-ignore (default: 0.01)")
    parser.add_argument("--beacon_grid_size", type=int, default=100,
                      help="Resolution of the h-index DP look-up table (default: 100)")
    ...
    parser.add_argument("--adabon_k0", type=int, default=3,
                      help="AdaBoN exploration budget: initial samples per prompt (default: 3)")
    parser.add_argument("--adabon_budgets", type=int, nargs='+', default=None,
                      help="AdaBoN total budget values to sweep (default: [4, 8, 16, n_total])")
    parser.add_argument("--adabon_difficulty", type=str, default="variance",
                      choices=["variance", "max_reward", "combined"],
                      help="AdaBoN difficulty estimator (default: variance)")
    ...
    try:
        ...
        results = comparison.compare_methods(
            ...
            adabon_k0=args.adabon_k0,
            adabon_budgets=args.adabon_budgets,
            adabon_difficulty=args.adabon_difficulty,
        )
        ...
        print(f"\nAdaBoN (baseline, arXiv 2505.12050):")
        for b in sorted(results["adabon"].keys()):
            print(f"  Budget={b}:")
            print(f"    Accuracy: {results['adabon'][b]['accuracy']:.3f}")
            print(f"    Avg Reward: {results['adabon'][b]['avg_reward']:.3f}")
            print(f"    Avg Samples: {results['adabon'][b]['samples_used']:.2f}")
            print(f"    Value Function: {results['adabon'][b]['value']:.3f}")
