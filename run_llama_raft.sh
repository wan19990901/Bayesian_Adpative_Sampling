...
base_path="./iter_dpo_raft_reward"
mkdir $base_path
...
run_iteration() {
    ...
    python reward_labeling.py --dataset_name_or_path "${output_dir}_data.jsonl" --output_dir $model_output --use_nemotron --use_rise
   
    conda activate rlhflow
    cat <<EOT > llama_raft_config.yaml
run_name: $iteration
...
do_eval: true
eval_steps: 10000
choose_type: max_random  # Using max_random for RAFT (best vs random)
...
EOT
    ...
    accelerate launch --config_file ./configs/zero3.yaml dpo_iteration/run_dpo.py llama_raft_config.yaml
}
...
for i in {1..9}
do
    iteration_name="Llama_raft_iter${i}"
    ...
    if [ $i -eq 1 ]; then
        ...
        model_path="Llama_raft_iter${previous_iteration}"
    fi
